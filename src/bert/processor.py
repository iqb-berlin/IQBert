...
def prepare(text: str) -> { "sentences": list[str], "vocab_size": int }:
...
def make_batch(text: str, maxlen: int, batch_size: int, max_pred: int) -> list[list[list[str | int] | list[int] | bool]]:
    batch = []
    positive = negative = 0
    while positive != batch_size / 2 or negative != batch_size / 2:
        tokens_a_index, tokens_b_index = randrange(len(sentences)), randrange(len(sentences))

        tokens_a, tokens_b = token_list[tokens_a_index], token_list[tokens_b_index] # TODO rename to sentence_a and sentence_b ?
